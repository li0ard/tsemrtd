...
import { COM, DG1, DG2, DG3, DG5, DG7, DG11, DG12, SOD, DG15, DG4, DG14, Schemas, Utils } from "../src"
...
test("DG2", async () => {
    const data = DG2.load(await getDGContent("EF_DG2.bin"));
    expect(
...
test("DG3", async () => {
    const data = DG3.load(await getDGContent("EF_DG3.bin"));
    expect(
        new Bun.CryptoHasher("sha256").update(data[0].imageData).digest().toString("hex")
...
test("DG4", async () => {
    const data = DG4.load(await getDGContent("EF_DG4.bin"));
    expect(
        new Bun.CryptoHasher("sha256").update(data[0].imageData).digest().toString("hex")
